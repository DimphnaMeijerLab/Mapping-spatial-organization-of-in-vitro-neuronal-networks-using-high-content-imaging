...
// Additions by Jurriaan Schuring and Myta Joosten
//
...
#@ String (label="Wells you want to stitch (seperaed by commas)") well_string
...
for (l = 0; l < well_list.length; l++) {
	...
	if (File.exists(output_file)){
		showMessageWithCancel("WARNING","Warning: fused image is already created for well " + well + "!\n Do you want to continue and overwrite the old fused image?");
	}
}

// Subsequent checks and dialog done only once with info from
// The last Well in the list
...
yes_no2=newArray("CONFIRM", "TOP","LEFT", "CENTER","RIGHT","BOTTOM","NO");
// Dialog -------------------------------------------------------------------------
...
Dialog.addString("Names of seperate channels (separated by commas)", "dapi,map2,atubulin,AnkG");
...
if (nr_th_channels > 0){
	...
	Dialog.setInsets(20, 0, 0);
	Dialog.addString("Names of thresholded channels (separated by commas)", "dapi,map2,atubulin,AnkG");
}

Dialog.setInsets(10, 0, 0);
Dialog.addChoice("CellposeRGB?", yes_no, yes_no[1]);
...
Dialog.setInsets(10, 0, 0);
Dialog.addChoice("Automode?", yes_no2);
Dialog.addNumber("Size of the circular selection? (diameter in no. of tiles)", 11);
...
suggested_ch_names_str = Dialog.getString();
suggested_ch_names = split(suggested_ch_names_str, ",");

// Check if the number of suggested threshold names matches the number of selected channels
nr_selected_channels = 0;
for (i = 0; i < stitch_channels.length; i++) {
	nr_selected_channels = nr_selected_channels + stitch_channels[i];
}
if (suggested_ch_names_str==""){
	if (nr_selected_channels != 0) {
		exit("Sorry, you selected channels without suggesting a name for them.");
	}
}
else{
	if (nr_selected_channels != suggested_ch_names.length) {
		exit("Sorry, the number of selected channels (" + d2s(nr_selected_channels,0) + ") \ndoes not match the given number of Channel names (" + suggested_ch_names_str + ").");
	}
}
...
if (nr_th_channels > 0){ // if there are images in the threshold folder
	...
	suggested_th_names_str = Dialog.getString();
	suggested_th_names = split(suggested_th_names_str, ",");
...
// get stich and th channels for metadata
...
// Save parameters to metadata file for all wells
for (l = 0; l < well_list.length; l++) {
	well=well_list[l];
	// Follows ----------------------------------------------------------------------
	well_folder = root + q + well;
	...
	if (stitch_th_channels_str != "None"){
		metadata = metadata + "NamesOfThresholdedChannels = " + suggested_th_names_str + "\n";
	}
	metadata = metadata + "CellposeRGB = " + Cellpose + "\n";
	...
	metadata = metadata + "Automode = " + automode + "\n";
	metadata = metadata + "Ovalsize = " + ovaltiles + "\n";
...
for (l = 0; l < well_list.length; l++) {
	...
	rename(("Fused-c" + d2s(ol_channel, 0)).c_str());
	...
	if (automode == "NO") {
		...
		Dialog.createNonBlocking("Stitching result");
	...
	if (automode == "NO") {
		tilesize = (widthImg/w+heightImg/h)/2;
		Ovalsize = tilesize*ovaltiles;
		locX = (widthImg - Ovalsize) / 2;
		locY = (heightImg - Ovalsize) / 2;
		makeOval(locX, locY, Ovalsize, Ovalsize);
		title2 = "Make oval selection";
		message2 = "\nFit a circular selection to the well borders. Hold shift when resizing.\nPress OK when you are done.";
		waitForUser(title2, message2);
		setBatchMode(true);
	}
	else if (automode == "CONFIRM") {
		setBatchMode("exit and display");
		tilesize = (widthImg/w+heightImg/h)/2;
		Ovalsize = tilesize*ovaltiles;
		locX = (widthImg - Ovalsize) / 2;
	...
	else if (automode == "TOP") {
		tilesize = heightImg/h;
		Ovalsize = tilesize*ovaltiles;
		locX = (widthImg - Ovalsize) / 2;
		locY = (heightImg - Ovalsize) / 2 - tilesize;
		makeOval(locX, locY, Ovalsize, Ovalsize);
	}
	else if (automode == "LEFT") {
		tilesize = widthImg/w;
		Ovalsize = tilesize*ovaltiles;
		locX = (widthImg - Ovalsize) / 2 - tilesize;
		locY = (heightImg - Ovalsize) / 2;
		makeOval(locX, locY, Ovalsize, Ovalsize);
	}
	else if (automode == "CENTER") {
		tilesize = (widthImg/w+heightImg/h)/2;
		Ovalsize = tilesize*ovaltiles;
		locX = (widthImg - Ovalsize) / 2;
		locY = (heightImg - Ovalsize) / 2;
	...
	for (i=1; i<channels_to_stitch.length+1; i++){
		index=channels_to_stitch.length-i;
		run("Duplicate...", "duplicate channels="+i);
		saveAs(root+q+well+q+well+"_"+suggested_ch_names[index]+".tif");
		selectWindow(well+"_fused.tif");
	}

	if (Cellpose=="Yes"){
		print("Converting to 8-bit RGB and creating cellpose image...");
		...
		run("Red");
		run("RGB Color");
		saveAs(root+q+well+q+well+"_Cellpose_RGB.tif");
	}
	else{
	...
	if (th_channels_to_stitch.length > 0){
		...
		for (i = 0; i < th_channels_to_stitch.length; i++) {
			...
			rename(well+"_th_"+suggested_th_names[i]);

			//fix MIST artifacts on image seams in the thresholded image
			...
			if(s=="Yes"){
				...
				// ch_name = getString("Name of this channel:", suggested_th_names[i]);
				ch_name = suggested_th_names[i];
				roiManager("Select", 0);
				run("Crop");
				run("Clear Outside");
...
print("\n>>> SUCCESSFULLY STITCHED ALL WELLS");
