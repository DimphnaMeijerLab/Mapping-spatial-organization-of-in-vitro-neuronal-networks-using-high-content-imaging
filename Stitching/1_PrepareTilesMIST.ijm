...
#@ String (label="Which wells are Dense Wells? (Yes or No, separated by commas)") dense_well
...
dense_list = split(dense_well, ",");
// Check if enough DenseWell is entered
if (!(dense_list.length == well_list.length)){
	exit("Sorry, your Dense Well input is wrong.\nThe input length does not match the amount of wells that you asked me to process.\nI was given: \n" + dense_well + "\nMake sure to match the amount of wells like so:\nWells:B08,C08,D08\nDense Wells: Yes,No,Yes");
}
...
for (l = 0; l < well_list.length; l++) {
	...
	// Check if DenseWell is entered correctly
	if (!(dense_list[l] == "Yes" || dense_list[l] == "No")){
		exit("Sorry, your Dense Well input is wrong. I was given\n" + dense_well + "\nbut it should be it in the format of: Yes,No,Yes");
	}
...
Dialog.setInsets(40, 0, 0);
Dialog.addMessage("On which channels do you want to set a normal threshold?");
Dialog.setInsets(0, center_inset, 0);
Dialog.addCheckboxGroup(1, nr_channels, ch_labels, ch_defaults);
...
Dialog.setInsets(40, 0, 0);
Dialog.addMessage("On which channels do you want to set an adaptive threshold?");
...
ch_adaptive_threshold = get_next_checkbox_group(nr_channels);

// Check whether the number of threshold methods provided is correct.
threshold_all_methods=newArray("Default", "Huang", "Huang2", "Intermodes", "IsoData", "Li", "MaxEntropy", "Mean", "MinError(I)", "Minimum", "Moments", "Otsu", "Percentile", "RenyiEntropy", "Shanbhag", "Triangle", "Yen");
...
if ((threshold_method_list[0] == "-") && num_threshold_channels != 0) {
	exit("Please enter the thresholding methods for the channels you want to threshold.\n \nExample:\n number of channels = 2\n threshold methods = Li,Otsu");
}
...
if ((threshold_method_list.length != num_threshold_channels) && (threshold_method_list[0] != "-")) {
 ...
 //Check if methods are in threshold all methods list
for (m = 0; m<threshold_method_list.length; m++) {
	contains = false;
	for (n = 0; n<threshold_all_methods.length; n++) {
		if (threshold_method_list[m] == threshold_all_methods[n]) {
			contains = true;
			break
		}
	}
	if (!contains) {
		exit("The threshold methods you have entered do not match with the available threshold methods.\n \nPlease enter a method from the following list:\n \nDefault, Huang, Huang2, Intermodes, IsoData\nLi, MaxEntropy, Mean, MinError(I)\nMinimum, Moments, Otsu, Percentile\nRenyiEntropy, Shanbhag, Triangle, Yen")
	}
}
...
ch_adaptive_threshold_str = get_true_indeces_as_string(ch_adaptive_threshold);
...
for (l = 0; l < well_list.length; l++) {
	...
	metadata = metadata + "ChannelsAdaptiveThreshold = " + ch_adaptive_threshold_str + "\n";
	metadata = metadata + "DenseWell = " + dense_list[l] + "\n";
	...
	for (i = 0; i < w*h; i++) {
		...
		for (j = 0; j < nr_channels; j++){
			...
			if ((ch_enhance[j] == true) && (ch_equalize[j] == false)){
				...
				run("Duplicate...", " ");
				rename(current_tile + "_CE");
			}
			if ((ch_enhance[j] == true) && (ch_equalize[j] == true)){
				selectWindow(current_tile);
				run("Duplicate...", " ");
				rename(current_tile + "_CE");
				selectWindow(current_tile + "_CE");
				run("Enhance Contrast...", "saturated="+d2s(fraction_saturated,1)+" normalize");
			...
			if (ch_threshold[ch] == true || ch_adaptive_threshold[ch] == true){
				...
				if (ch_threshold[ch] == true){
					th_method = threshold_method_list[normal_th_counter];
					selectWindow(current_tile);
					run("Duplicate...", " ");
				...
				if (ch_adaptive_threshold[ch] == true){
					if (dense_list[l] == "Yes"){
						selectWindow(current_tile);
						run("Anisotropic Diffusion 2D", "number=30 smoothings=4 keep=30 a1=0.75 a2=0.95 dt=20 edge=5");
						rename("th_" + d2s(threshold_counter+1,0));
						run("Gaussian Blur...", "sigma=2");
						run("Auto Local Threshold", "method=Mean radius=20 parameter_1=-1.5 parameter_2=0 white");
